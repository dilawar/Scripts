...
def connect():
    server = ldap.Server('ldap.ncbs.res.in:389', get_info=ldap.ALL)
...
def search(query, connection=None):
    ...
    try:
        ...
        if query:
            post = '*'
        filterstr = '(|%s)' % (u' '.join([
            u'(%s=*%s%s)' % (field, query, post)
            for field in ['givenName', 'uid', 'mail']
        ]))
        print(filterstr)
        connection.search('dc=ncbs,dc=res,dc=in', filterstr.encode('utf-8'))
    finally:
...
if __name__ == '__main__':
    ...
    print(entries)
    addresses = [format_entry(e) for e in entries]
